  ...
  for (unsigned i=i_start; i<i_end; i++) {
	...
	for(int isdom=_iproc; isdom<_iproc+1; isdom++) {
	  for (int iel=_ml_msh->GetLevel(ig)->IS_Mts2Gmt_elem_offset[isdom]; 
	    ...
	    unsigned nloc_dof= _ml_msh->GetLevel(ig)->el->GetElementDofNumber(kel_gmt,solType);
	  
            for(int j=0; j<nloc_dof; j++) {
	      unsigned inode=_ml_msh->GetLevel(ig)->el->GetMeshDof(kel_gmt,j,solType);
	      unsigned inode_Metis=_ml_msh->GetLevel(ig)->GetMetisDof(inode,solType);
	      ...
	      double value = ( initFuncion == NULL ) ? 0. : initFuncion(xx,yy,zz);
